...
void LefVia::setLayerIdx(const Index_t i, const Index_t v) {
  assert(0 <= i and i <= 2);
  _layerIndices[i] = v;
}

void LefVia::setLayerName(const Index_t i, const String_t& n) {
  assert(0 <= i and i <= 2);
  _layerNames[i] = n;
}

void LefVia::addBox(const Index_t i, const Box<Int_t>& b) {
  assert(0 <= i and i <= 2);
...
void LefVia::logInfo() const {
  ...
  for (Index_t i = 0; i < _vBoxes[0].size(); ++i) {
    Box<Int_t> box = _vBoxes[0][i];
    fprintf(fout, "    RECT %d %d %d %d\n", box.xl(), box.yl(), box.xh(), box.yh());
  }
  fprintf(fout, "  LAYER %s\n", _layerNames[1].c_str());
  for (Index_t i = 0; i < _vBoxes[1].size(); ++i) {
    Box<Int_t> box = _vBoxes[1][i];
    fprintf(fout, "    RECT %d %d %d %d\n", box.xl(), box.yl(), box.xh(), box.yh());
  }
  fprintf(fout, "  LAYER %s\n", _layerNames[2].c_str());
  for (Index_t i = 0; i < _vBoxes[2].size(); ++i) {
